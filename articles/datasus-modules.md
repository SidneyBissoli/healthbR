# DATASUS Modules: SIM, SINASC, SIH, and SIA

## Overview

The `healthbR` package provides access to four DATASUS information
...
- **Use `month`** (SIH and SIA) to limit monthly downloads. Downloading
  a full year for all states requires 324 files per module (27 UFs x 12
  months).
...
- SIH and SIA are monthly, so a full-year download is 324 files per
  type. SIA has 13 file types, so downloading everything for one year
  would be 4,212 files – always filter by `type`, `uf`, and `month`.
