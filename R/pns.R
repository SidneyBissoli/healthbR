...
pns_sidra_search <- function(keyword, year = NULL) {
  ...
  keyword_norm <- tolower(keyword)
  # remove accents using base R
  keyword_norm <- iconv(keyword_norm, from = "UTF-8", to = "ASCII//TRANSLIT")

  # search in table names
  catalog <- catalog |>
    dplyr::mutate(
      table_name_norm = iconv(tolower(.data$table_name), from = "UTF-8", to = "ASCII//TRANSLIT")
    ) |>
    dplyr::filter(
      stringr::str_detect(.data$table_name_norm, stringr::fixed(keyword_norm))
    ) |>
    dplyr::select(-"table_name_norm")
